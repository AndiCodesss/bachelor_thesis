...
from app.services.interpretation.rule_engine import (
    ENTRY_PATTERNS,
    RuleBasedTradeInterpreter,
    _compact_repeated_trade_text,
    _resolve_price,
)
...
def test_entry_at_pattern_ignores_stop_at() -> None:
    # M4: the entry "at" pattern must not capture the price in a "stop at N"
    # clause (which would corrupt the entry price with the stop level).
    entry_at = re.compile(ENTRY_PATTERNS[0])
    assert entry_at.search("long here at 46") is not None
    assert entry_at.search("stop at 36") is None
    assert entry_at.search("stops at 36") is None


def test_detect_trim_quantity_never_resolves_to_full_close() -> None:
    # M2: a trim is a partial exit -- "all"/"whole position" must be clamped so a
    # take-some cue can never liquidate the entire position.
    interpreter = RuleBasedTradeInterpreter()
    assert interpreter._detect_trim_quantity("trimming the whole position") == "most"
    assert interpreter._detect_trim_quantity("take it all off here") == "most"
    assert interpreter._detect_trim_quantity("taking half off") == "half"
    # The base detector is unchanged (full close still recognised for EXIT_ALL).
    assert interpreter._detect_quantity("whole position") == "all"


def test_resolve_price_recovers_spoken_separator_zero() -> None:
    # M5: "nineteen four oh" is 1-9-4-0 = 1940; the trailing "oh" zero must not
    # be silently dropped.
    assert _resolve_price("nineteen four oh", 1942.0) == 1940.0
...
async def test_rule_interpreter_detects_imperative_pay_yourself_trim() -> None:
    interpreter = RuleBasedTradeInterpreter()
    session = StreamSession(
        config=SessionConfig(),
...
async def test_rule_interpreter_ignores_advisory_trim() -> None:
    interpreter = RuleBasedTradeInterpreter()
    session = StreamSession(
        config=SessionConfig(),
...
async def test_rule_interpreter_ignores_advisory_entry() -> None:
    interpreter = RuleBasedTradeInterpreter()
...
async def test_rule_interpreter_ignores_looking_for_the_short() -> None:
    interpreter = RuleBasedTradeInterpreter()
    session = StreamSession(config=SessionConfig(), market=MarketSnapshot(symbol="MNQ 03-26", last_price=25000.0))
...
async def test_rule_interpreter_ignores_interested_in_the_short() -> None:
    interpreter = RuleBasedTradeInterpreter()
    session = StreamSession(config=SessionConfig(), market=MarketSnapshot(symbol="MNQ 03-26", last_price=25000.0))
...
async def test_rule_interpreter_ignores_tempted_to_put_piece_on() -> None:
    interpreter = RuleBasedTradeInterpreter()
    session = StreamSession(config=SessionConfig(), market=MarketSnapshot(symbol="MNQ 03-26", last_price=25000.0))
...
async def test_rule_interpreter_ignores_looking_to_get_piece_on() -> None:
    interpreter = RuleBasedTradeInterpreter()
    session = StreamSession(config=SessionConfig(), market=MarketSnapshot(symbol="MNQ 03-26", last_price=25000.0))
...
async def test_rule_interpreter_does_not_extract_intent_from_fallback_without_rule_match() -> None:
    class _Fallback:
    ...
    session = StreamSession(config=SessionConfig(enable_ai_fallback=True), market=MarketSnapshot(symbol="NQ", last_price=21250.0))
...
async def test_rule_interpreter_uses_gemini_confirmation_to_veto_entry() -> None:
    class _Fallback:
        def __init__(self) -> None:
            self.interpret = AsyncMock()
            self.confirm_intent = AsyncMock()
    ...
    interpreter = RuleBasedTradeInterpreter(fallback=mock_fallback)
    session = StreamSession(config=SessionConfig(enable_ai_fallback=True), market=MarketSnapshot(symbol="MNQ 03-26", last_price=25000.0))
...
async def test_rule_interpreter_allows_strong_entry_when_gemini_confirmation_times_out() -> None:
    ...
    session = StreamSession(
        config=SessionConfig(enable_ai_fallback=True),
...
async def test_rule_interpreter_does_not_use_extractive_fallback_without_rule_intent() -> None:
    class _Fallback:
        def __init__(self) -> None:
            self.interpret = AsyncMock()
            self.confirm_intent = AsyncMock()
