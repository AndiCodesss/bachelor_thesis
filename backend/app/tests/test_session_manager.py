...
from app.services.execution.ninjatrader import BridgeUnavailableError
...
async def test_uncertain_preview_that_did_not_fill_does_not_swallow_confirmed_final(tmp_path: Path) -> None:
    manager, bridge = _make_broker_backed_manager(tmp_path, last_price=25045.5, default_contract_size=1)
    session = await manager.create_session(
        CreateSessionRequest(config=SessionConfig(enable_early_preview_entries=True, default_contract_size=1))
    )

    original_post_command = bridge.post_command
    calls = {"post": 0}

    async def first_post_times_out(payload: dict[str, object]) -> httpx.Response:
        calls["post"] += 1
        if calls["post"] == 1:
            request = httpx.Request("POST", "http://127.0.0.1:18080/api/stream-copier/commands")
            return httpx.Response(
                status_code=400,
                json={"ok": False, "code": "dispatch_timeout", "message": "dispatch timed out"},
                request=request,
            )
        return await original_post_command(payload)

    manager._bridge_client.post_command = first_post_times_out  # type: ignore[method-assign]
    ...
    assert manager.get_session(session.id).position is None
    ...
    assert updated.position is not None
    assert updated.position.quantity == 1
    assert any(event.title == "Preview entry not found" for event in updated.events)
    assert not any(event.title == "Preview flatten" for event in updated.events)
...
async def test_config_patch_ignores_null_boolean_fields(tmp_path: Path) -> None:
    manager = SessionManager(Settings(data_dir=tmp_path))
    session = await manager.create_session(
        CreateSessionRequest(
    ...
    updated = await manager.update_session_config(
        session.id,
        UpdateSessionConfigRequest.model_validate(
            {
                "enable_partial_intent_detection": None,
                "enable_ai_fallback": None,
                "enable_early_preview_entries": None,
            }
        ),
    )

    assert updated.config.enable_partial_intent_detection is True
    assert updated.config.enable_ai_fallback is True
    assert updated.config.enable_early_preview_entries is True
...
async def test_manual_trade_applies_broker_overrides_before_execution(tmp_path: Path) -> None:
    ...
    async def fake_execute(current_session, intent, *, contract_size=None):
        ...
        return await original_execute(current_session, intent, contract_size=contract_size)
...
async def test_manual_add_within_cap_succeeds_through_risk_gate(tmp_path: Path) -> None:
    # A manual add that stays within max_contract_size (default 4) still goes
    # through the risk gate and is approved, scaling the position.
    manager, _bridge = _make_broker_backed_manager(tmp_path, last_price=21243.75)
    session = await manager.create_session(CreateSessionRequest(config=SessionConfig()))
    await manager.manual_trade(session.id, ManualTradeRequest(action=ManualTradeAction.buy, contract_size=3))
    ...
    updated = await manager.manual_trade(
        session.id,
        ManualTradeRequest(action=ManualTradeAction.buy, contract_size=1),  # 3+1 == cap
    )

    assert updated.position is not None
    assert updated.position.side == TradeSide.long
    assert updated.position.quantity == 4
    assert updated.position.stop_price == 21123.75
    assert updated.position.target_price == 21483.75


async def test_manual_entry_blocked_when_exceeding_max_contract_size(tmp_path: Path) -> None:
    # H1: manual trades are not a privileged bypass -- the size cap still applies.
    manager, _bridge = _make_broker_backed_manager(tmp_path, last_price=21243.75)  # max_contract_size defaults to 4
    session = await manager.create_session(CreateSessionRequest(config=SessionConfig()))

    calls = {"execute": 0}
    original_execute = manager._executor.execute

    async def counting_execute(current_session, intent, *, contract_size=None):
        calls["execute"] += 1
        return await original_execute(current_session, intent, contract_size=contract_size)
    ...
    updated = await manager.manual_trade(
        session.id,
        ManualTradeRequest(action=ManualTradeAction.buy, contract_size=5),  # 0+5 > max 4
    )

    assert updated.position is None
    assert calls["execute"] == 0
    risk_events = [event for event in updated.events if event.title == "Risk check"]
    assert risk_events and risk_events[-1].data["approved"] is False


async def test_concurrent_manual_buys_are_serialised_into_one_entry(tmp_path: Path) -> None:
    # H2: the per-session lock prevents two concurrent buys from both seeing a
    # flat position and each opening a fresh entry (which would double exposure).
    manager, _bridge = _make_broker_backed_manager(tmp_path, last_price=21243.75)
    session = await manager.create_session(CreateSessionRequest(config=SessionConfig()))
    ...
    await asyncio.gather(
        manager.manual_trade(session.id, ManualTradeRequest(action=ManualTradeAction.buy, contract_size=3)),
        manager.manual_trade(session.id, ManualTradeRequest(action=ManualTradeAction.buy, contract_size=3)),
    )

    stored = manager.get_session(session.id)
    assert stored.position is not None
    # The first buy opens the position; the second is serialised behind it, sees
    # the open long, becomes an ADD, and is capped (3+3 > 4) -- so it never fires.
    assert stored.position.quantity == 3
...
async def test_bridge_unavailable_is_cached_and_pipeline_keeps_running(tmp_path: Path) -> None:
    """With the bridge unreachable the app keeps working: the 'unavailable' state
    is cached so repeated syncs don't re-pay the connect timeout, and ingesting a
    segment never raises -- trades simply aren't copied."""
    manager = SessionManager(Settings(data_dir=tmp_path, default_symbol="NQ", ninjatrader_symbol=""))
    session = await manager.create_session(CreateSessionRequest(config=SessionConfig(symbol="NQ")))
    ...
    async def fake_fetch(*, account, symbol):
        nonlocal calls
        calls += 1
        raise BridgeUnavailableError("connect timeout")

    manager._bridge_client.fetch_state = fake_fetch  # type: ignore[method-assign]

    first = await manager.get_broker_state(session.id)
    assert first["ok"] is False
    assert first["code"] == "bridge_unavailable"

    # A non-trade segment triggers a broker sync; it must not raise, and it must be
    # served from the negative cache instead of probing the dead bridge again.
    await manager.ingest_segment(session.id, TextSegmentRequest(text="just commentary"))
    second = await manager.get_broker_state(session.id)
    assert second["code"] == "bridge_unavailable"

    # Only the first probe reached the (dead) bridge; everything else hit the cache.
    assert calls == 1
