...
async def test_ninjatrader_executor_posts_command() -> None:
    ...
    response = httpx.Response(status_code=200, json={"ok": True, "message": "accepted"}, request=request)
...
async def test_ninjatrader_executor_reports_transport_failure() -> None:
    ...
    # A transport failure is uncertain: the order may already be in flight.
    assert result.uncertain is True
    assert "bridge offline" in result.message


async def test_ninjatrader_executor_treats_unconfirmable_2xx_as_uncertain() -> None:
    # Fail-closed: a 2xx whose body confirms neither ok nor an order id must not
    # be treated as an accepted order; it is uncertain so the caller reconciles.
    request = httpx.Request("POST", "http://127.0.0.1:18080/api/stream-copier/commands")
    response = httpx.Response(status_code=200, text="accepted", request=request)
    executor = NinjaTraderExecutor(
        Settings(),
...
async def test_ninjatrader_executor_treats_dispatch_timeout_as_uncertain() -> None:
    # The bridge timed out dispatching but the order may still have been
    # submitted -> uncertain (reconcile), not a definitive rejection.
    request = httpx.Request("POST", "http://127.0.0.1:18080/api/stream-copier/commands")
...
async def test_ninjatrader_executor_accepts_order_id_without_ok() -> None:
    request = httpx.Request("POST", "http://127.0.0.1:18080/api/stream-copier/commands")
    response = httpx.Response(status_code=200, json={"orderId": "abc-123"}, request=request)
    executor = NinjaTraderExecutor(
        Settings(),
        bridge_client=_FakeBridgeClient(response=response),  # type: ignore[arg-type]
    )
...
async def test_bridge_client_fetch_state_forwards_query_params(monkeypatch) -> None:
    settings = Settings(
        ...
        ninjatrader_bridge_timeout_ms=6_000,  # explicit so the test is independent of any local .env
    )
    ...
    # Connect is capped (so an absent bridge fails fast) while read/write/pool keep
    # the full configured budget for actual order dispatch.
