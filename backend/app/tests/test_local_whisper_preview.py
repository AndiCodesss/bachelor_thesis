...
def test_transcription_profiles_use_different_beam_sizes() -> None:
    ...
    preview_text, _ = transcriber._transcribe_with_profile(model, b"\x00\x00" * 20, beam_size=1, decode_profile="preview")
...
def test_preview_transcription_uses_preview_profile_without_final_guardrails() -> None:
    ...
    text, confidence = transcriber._transcribe_with_profile(_FakeModel(), b"\x00\x00" * 160, beam_size=1, decode_profile="preview")
...
def _ready_segment(utterance_id: str) -> ReadyAudioSegment:
    return ReadyAudioSegment(
...
async def test_worker_survives_a_per_utterance_failure(monkeypatch) -> None:
    # H5: a single utterance failing must not permanently kill the worker.
    emitted: list[str] = []

    async def record(segment) -> None:
    ...
    def fake_transcribe(_model: object, _pcm16: bytes) -> tuple[str, float]:
        calls["n"] += 1
        if calls["n"] == 1:
            raise RuntimeError("transient decode failure")
        return "second utterance", 0.9

    monkeypatch.setattr(transcriber, "_ensure_model", fake_ensure_model)
    monkeypatch.setattr(transcriber, "_transcribe_final_segment", fake_transcribe)

    transcriber._queue.put_nowait(_ready_segment("one"))
    transcriber._queue.put_nowait(_ready_segment("two"))
    transcriber._queue.put_nowait(None)  # shutdown sentinel

    await asyncio.wait_for(transcriber._worker(), timeout=5)

    # The first utterance raised, but the worker kept going and emitted the second.
    assert calls["n"] == 2
    assert emitted == ["second utterance"]


async def test_preview_failure_fails_open(monkeypatch) -> None:
    # H6: a preview failure must never poison the transcriber or block finals.
    transcriber = LocalWhisperTranscriber(
        settings=Settings(),
        session_id="session",
        model_name="small.en",
    ...
    monkeypatch.setattr(transcriber, "_ensure_preview_model", boom)

    # Must swallow the error rather than propagate it...
    await transcriber._run_preview(_ready_segment("abc"), transcriber._preview_generation)

    # ...and leave no sticky error state that could disable real transcription.
    assert not hasattr(transcriber, "_last_error")
