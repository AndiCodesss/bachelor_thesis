...
class Settings(BaseSettings):
    ...
    max_transcript_segments: int = Field(default=250, ge=1)
    max_events: int = Field(default=500, ge=1)
    default_symbol: str = "NQ"

    transcription_backend: str = "local_whisper"
    # Default ASR model. Kept in sync with SessionConfig.transcription_model (the
    # per-session value the live pipeline actually uses) so config and runtime agree.
    # distil-small.en is fast enough to keep up with live speech; larger models
    # (small.en, distil-large-v3) are selectable per session from the dashboard.
    ...
    transcription_cpu_threads: int = Field(default=4, ge=1)
    ...
    gemini_confirmation_timeout_ms: int = Field(default=1_500, ge=1)

    min_confidence: float = Field(default=0.74, ge=0.0, le=1.0)
    max_entry_distance_points: float = Field(default=12.0, ge=0.0)
    max_entry_signal_age_ms: int = Field(default=20_000, ge=0)
    preview_entry_confirmation_window_ms: int = Field(default=12_000, ge=0)
    max_entry_chase_points: float = Field(default=8.0, ge=0.0)
    entry_context_window_ms: int = Field(default=15_000, ge=0)
    entry_guard_window_ms: int = Field(default=20_000, ge=0)
    force_wide_brackets: bool = True
    wide_stop_points: float = Field(default=120.0, ge=0.0)
    wide_target_points: float = Field(default=240.0, ge=0.0)
    stale_intent_ms: int = Field(default=5_000, ge=0)
    default_contract_size: int = Field(default=1, ge=1)
    max_contract_size: int = Field(default=4, ge=1)
    ninjatrader_bridge_url: str = "http://127.0.0.1:18080"
    ninjatrader_bridge_token: str | None = None
    # Must exceed the bridge's dispatch budget (DispatchTimeoutMs, default 5000)
    # so the client never gives up while the bridge is still placing the order.
    ninjatrader_bridge_timeout_ms: int = Field(default=6_000, ge=1)
    ...
    def normalize_legacy_transcription_config(self) -> "Settings":
        ...
        if self.default_contract_size > self.max_contract_size:
            raise ValueError(
                "DEFAULT_CONTRACT_SIZE must not exceed MAX_CONTRACT_SIZE "
                f"({self.default_contract_size} > {self.max_contract_size})"
            )
        return self
