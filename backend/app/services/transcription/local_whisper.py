...
# Maximum number of finalized utterances allowed to wait for final transcription.
# This is deliberately SMALL. Combined with the drop-oldest policy in push_audio,
# the cap is what bounds end-to-end *latency*, not just memory. A deep queue (this
# was originally 20) lets a fast speaker's utterances pile up so each final is
# emitted tens of seconds late -- useless for live trading, where the rule engine
# can only stitch fragments within ~6s and the risk engine vetoes stale signals.
# Capping at a couple of utterances bounds the worst-case wait to a few seconds;
# under sustained overload the oldest (stalest) finals are dropped, which is the
# right trade-off because they would be rejected downstream anyway.
_QUEUE_MAX_SEGMENTS = 3
...
class LocalWhisperTranscriber(BaseTranscriber):
    ...
    def __init__(
        ...
        *,
        ...
        self._dropped_segments = 0     # finalized utterances shed under backlog (for logging)
    ...
    async def push_audio(self, data: bytes, sample_rate: int) -> None:
        ...
        for segment in ready_segments:
            ...
            # Real-time backpressure: if the worker has fallen behind (queue full),
            # drop the OLDEST queued utterance to admit this newer one. The stalest
            # final is the least useful -- by the time it would be transcribed it is
            # already past the rule engine's stitching window and the risk engine's
            # staleness guard, so keeping it would only add latency for no benefit.
            if self._queue.full():
                try:
                    self._queue.get_nowait()
                    self._dropped_segments += 1
                    # Under sustained fast speech this can fire repeatedly, so log
                    # only periodically -- enough to surface the backlog without
                    # flooding the logs.
                    if self._dropped_segments % 20 == 1:
                        _LOGGER.warning(
                            "Transcription backlog for session %s: dropped %d stale finalized "
                            "utterance(s) to stay current with live speech",
                            self._session_id,
    ...
    async def _worker(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                # A single utterance failing (transient decode error, a raise
                # from the downstream handler, etc.) must not kill the worker --
                # log it and keep transcribing subsequent utterances.
                _LOGGER.exception(
                    "Failed to transcribe an utterance for session %s; continuing",
                    self._session_id,
                )
            finally:
    ...
    async def _run_preview(self, snapshot: ReadyAudioSegment, generation: int) -> None:
        ...
        try:
            ...
            # The preview path is best-effort and must fail open: a preview
            # failure can never disable the authoritative final transcription.
            _LOGGER.debug(
                "Preview transcription failed for session %s; ignoring",
                self._session_id,
                exc_info=True,
            )
        finally:
    ...
    def _cuda_runtime_looks_available(self) -> bool:
        """True only when both cuBLAS 12 and cuDNN shared libs are on disk -- a cheap pre-flight before a potentially-crashing CUDA model load."""
        found_cublas = False
        found_cudnn = False
    ...
    def _candidate_cuda_library_dirs(self) -> list[Path]:
        """Deduplicated list of dirs where CUDA libs might live (system paths, LD_LIBRARY_PATH entries, nvidia pip-wheel locations) that feeds the availability probe."""
    ...
    def _collect_transcription_result(self, segments: list[Any]) -> tuple[str, float]:
        ...
        for segment in segments:
            ...
            # Accept numpy scalars too -- isinstance(x, float) misses np.float32,
            # which would silently drop confidence to the 0.85 default.
            if isinstance(avg_logprob, numbers.Real) and not isinstance(avg_logprob, bool):
    ...
    def _has_consecutive_repeated_ngram(
        ...
        *,
        ...
        """True when the same n-gram (ngram_size tokens) repeats back-to-back at least min_repeats times."""
        # Earliest start position that could still fit min_repeats copies of the n-gram.
