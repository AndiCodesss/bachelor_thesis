"""Standalone CLI utility that downloads a YouTube channel's captions via yt-dlp
and renders them as timestamped plain-text transcripts plus a manifest.json."""
...
def _pick_language_key(tracks: dict[str, Any], preferred_language: str) -> str | None:
    ...
    # live_chat tracks are chat logs, not captions, so drop them up front.
    candidates = [key for key in tracks if "live_chat" not in key.lower()]
    if not candidates:
        return None

    # Rank tracks: prefer the exact requested language, then language-region
    # variants (e.g. en-US), and de-prioritise auto-translated "orig" tracks.
...
def _append_caption_line(lines: list[CaptionLine], item: CaptionLine) -> None:
    ...
    if item.text == previous.text:
        return
    # YouTube auto-captions stream incrementally: each cue restates the previous
    # text plus a few new words. Collapse such near-duplicate consecutive lines by
    # replacing the previous line with the longer superset instead of appending it.
