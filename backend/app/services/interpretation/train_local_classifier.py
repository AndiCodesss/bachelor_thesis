"""Train the local ModernBERT intent-classifier head from reviewed execution labels.

End-to-end pipeline:
    load -> dedupe -> transcript-level split -> rebalance -> embed -> train linear
    head -> calibrate per-label thresholds.

We freeze the ModernBERT backbone and only fit a small linear head on top of its
mean-pooled embeddings, so the encoder is used purely as a feature extractor. The
split is done at the transcript level (not row level) to avoid leakage between
train/val/test, the NO_ACTION majority is rebalanced down, and a probability
threshold is calibrated per action label for inference-time gating.

Artifacts written to ``--artifact-dir``:
    * ``classifier_head.safetensors`` — the trained linear head weights.
    * ``classifier_metadata.json`` — backbone name/path, labels, calibrated
      thresholds, split/rebalance summaries, and validation/test metrics.

Usage example::

    python -m app.services.interpretation.train_local_classifier \\
        --dataset data/training_data_clean.jsonl --artifact-dir data
"""
...
def _split_examples_by_transcript(
    ...
    *,
    ...
    """Split examples into leakage-free train/val/test sets at the transcript level.

    Every row from the same source transcript stays in the same split, so the model
    is never validated/tested on a transcript it partly trained on. ``stable_hash``
    keeps the split deterministic across runs; ``temporal_recent`` instead holds out
    the newest transcripts. Returns ``(train, validation, test, split_summary)``.
    """
    ...
    # Reserve at least two transcripts (the -2) for train+val so the test holdout can
    # never swallow the whole corpus and leave nothing to train/validate on.
    ...
    # Prefer transcripts that actually contain action labels as test candidates, so
    # rare action classes are represented in the holdout instead of all NO_ACTION.
...
def _classification_metrics(*, true_labels: list[int], predicted_labels: list[int], labels: tuple[ActionTag, ...]) -> dict:
    ...
    # Collapse every action tag into one positive class to measure pure trigger/no-trigger
    # decision quality, independent of which specific action the model picked.
...
def _calibrate_thresholds(*, probabilities, true_labels: list[int], labels: tuple[ActionTag, ...]) -> dict[str, float]:
    """Return one F1-optimal probability threshold per action label.

    At inference these gate predictions (probability must clear the label's threshold)
    instead of trusting a plain argmax, which lets us tune precision/recall per action.
    Each label defaults to 0.55 if no bucket beats it, and the sweep covers 0.30–0.90
    in 0.02 steps — wide enough to cover useful operating points without overfitting
    the threshold to noise at the extremes.
    """
...
def _class_weights(torch_module: object, labels_tensor):
    """Inverse-frequency CrossEntropy class weights, with a zero-count guard to avoid divide-by-zero."""
