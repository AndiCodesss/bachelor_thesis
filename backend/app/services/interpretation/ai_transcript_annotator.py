"""Offline CLI tool that turns day-trading livestream transcripts into a labeled training set.

Transcripts are split into overlapping line chunks and sent to an AI annotator backend
(Gemini API, Claude CLI, or Gemini CLI) that marks the lines where an actionable trade
event happens. The labeled events are then emitted as a training dataset: a JSON report
plus JSONL examples consumable by the intent classifier. Run as a script via the
``_main`` entry point (``python -m ...`` / ``if __name__ == "__main__"``).
"""
...
class TranscriptChunkAnnotator(Protocol):
    """Backend contract: label one transcript chunk's actionable trade events."""
...
class _RateLimitedAnnotator:
    """Shared base that paces requests to a minimum interval so backends avoid rate limits."""
...
class _PromptCliAnnotator(_RateLimitedAnnotator):
    """Shared base for CLI-subprocess backends: pipes the prompt to an executable and retries with backoff."""
...
def build_training_examples(
    *,
    ...
    """Build training examples by replaying the transcript row-by-row through the rule interpreter.

    Replaying line-by-line means each annotated line's prompt is captured with the same
    recent-text/position context the live interpreter would have had at that point, so the
    training prompt matches what the model sees in production rather than full hindsight.
    """
...
def _align_annotation_row(
    *,
    ...
    # The AI often points a few lines off from where the rule engine actually fires, so
    # re-anchor its chosen line to the strongest rule-signal line within a +/-10 line window.
...
def _row_alignment_score(*, row: TranscriptRow, label: ActionTag, side: TradeSide | None) -> int:
    # Score how well this line matches the AI label per the rule engine; higher means a
    # stronger/more explicit signal match (0 = no match), so the caller can pick the best anchor.
    normalized = _normalize(row.text)
