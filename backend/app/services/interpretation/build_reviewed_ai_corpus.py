"""End-to-end builder for a reviewed AI transcript-training corpus.

The pipeline annotates each transcript with an LLM backend (Gemini CLI/API or
Claude CLI), caches a per-file report so reruns can skip already-processed
transcripts, aggregates those reports into a combined annotation file, runs an
automatic review pass over the candidates, and finally exports the accepted
decisions as ``reviewed_examples.jsonl`` for downstream training.
"""
...
def _safe_report_name(path: Path) -> str:
    """Build a report filename from the stem plus a 10-char sha256 suffix.

    The suffix keeps transcripts that share the same name across different
    directories from colliding onto a single cached report file.
    """
...
def _load_reviewed_coverage(inputs: Iterable[str]) -> set[str]:
    """Return the transcript paths already covered by prior reviewed corpora.

    Used to skip re-annotating transcripts a previous run already reviewed.
    Supports both the exported ``reviewed_examples.jsonl`` format and the raw
    ``combined_review.json`` state (counting only accepted/corrected candidates).
    """
    covered: set[str] = set()
...
def _export_combined_review(*, review_path: Path, jsonl_out: Path, summary_out: Path) -> dict:
    """Collapse reviewed decisions into final per-line training examples.

    Reads the review state, resolves the winning decision per transcript line,
    rebuilds the example text from the original rows, and writes the
    ``reviewed_examples`` jsonl alongside a summary of counts and drops.
    """
...
async def _main_async(args: argparse.Namespace) -> int:
    ...
    _aggregate_reports(report_paths=report_paths, combined_report_path=combined_report_path)
