"""Wrapper around a frozen ModernBERT encoder with a trained classification head.
...
class IntentClassifierPrediction:
    ...
    def action_probability(self) -> float:
        # _DEFAULT_LABELS is exactly the seven trained labels, so ActionTag.add
        # is intentionally absent here; the entry/management aggregates below
        # use broader label families that do include it.
        return sum(
...
class ModernBertIntentClassifier:
    ...
    def load(self) -> None:
        """Lazily load the classifier on first use; idempotent and never raises.

        We set _loaded before doing any work so a failed load is not retried on
        every call. Any failure (disabled, missing artifacts, import/load error)
        is deliberately captured into load_error()/runtime_info() rather than
        raised, so a missing or disabled classifier degrades gracefully.
        """
        if self._loaded:
            return
        self._loaded = True
    ...
    def classify(self, envelope: IntentContextEnvelope) -> IntentClassifierPrediction | None:
        ...
        with self._torch.inference_mode():
            ...
            # L2-normalize to match the training-time embedding pipeline
            # (train_local_classifier._embed_prompts): the head weights and the
            # calibrated per-label thresholds were fitted on unit-length
            # embeddings, so inference must feed the head the same scale.
            embeddings = self._torch.nn.functional.normalize(embeddings, p=2.0, dim=-1)
