"""Stage 2 of the annotation pipeline: curate raw AI annotations into a training set.

A CLI with three subcommands that operate on a persistent review-state JSON:

* ``review`` walks the AI candidates interactively for a human to label,
* ``auto`` auto-decides the obvious accept/reject/correct cases,
* ``export`` turns the curated decisions into classifier-ready JSONL.

Each candidate moves through the review state pending -> accepted/corrected/rejected
-> exported. Export keeps rejected lines too, emitting them as NO_ACTION hard
negatives so the classifier learns what *not* to fire on.
"""
...
# These patterns encode the precision rules from the annotator system prompt:
# a live first-person action ("I'm in this short") is a real trade and should be
# accepted, while advice / conditional / future language ("looking to sell",
# "I'll add on pops") gets rejected or downgraded to a setup.
...
def _auto_review_candidate(candidate: dict[str, Any]) -> bool:
    """Auto-decide a single candidate, leaving anything ambiguous as pending.

    Precision-first: we only touch cases the regex rules can call confidently, so a
    human still sees the rest. The checks run roughly in order of how clear-cut they
    are: reject other-instrument chatter first, then downgrade entries/adds to setups
    when the language is tentative, then accept explicit add/entry/trim/exit/stop
    confirmations. Returns True iff a decision was applied.
    """
...
def _collapse_reviewed_decisions(candidates: list[dict[str, Any]]) -> tuple[dict[str, dict[int, ReviewedLineDecision]], int]:
    """Pick one winning decision per (file, line) and count the rest as dropped.

    Overlapping transcript chunks mean the same line can show up as several reviewed
    candidates, so we collapse them: prefer a positive label over NO_ACTION, then the
    one with the highest status priority / confidence (see ``_decision_sort_key``).
    Returns the winners plus a count of the duplicates that were dropped.
    """
