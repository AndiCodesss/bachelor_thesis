...
def load_examples_grouped(
    ...
    no_action_ratio/max_no_action are unused here -- kept only for signature
    parity with the caller; rebalancing is deferred to the consumer (which folds
    the transcripts and rebalances per train split in cross_validate).
    """
    examples, _ = _load_examples(dataset)
    by_file: dict[str, list[dict]] = {}
    for ex in examples:
        by_file.setdefault(_example_file_key(ex), []).append(ex)
    # Sort by sha256 of the file key, not the raw name: gives a deterministic but
    # filename-order-independent shuffle so the contiguous fold chunking that
    # follows doesn't group transcripts by accidental naming adjacency.
...
def cross_validate(
    ...
    *,
    ...
    # Assign transcripts to folds using fixed-size contiguous chunks, then drop
    # any trailing EMPTY fold. ceil-sizing can leave an empty last fold for some
    # (n, k); previously it was silently skipped while the run still reported k
    # folds. Dropping empties keeps the partition identical when every fold is
    # non-empty and makes the reported fold count (len(folds)) honest otherwise.
    fold_size = math.ceil(len(file_keys) / k)
    folds: list[list[str]] = [file_keys[i * fold_size : (i + 1) * fold_size] for i in range(k)]
...
class TfidfMLP:
    """TF-IDF + two-hidden-layer MLP baseline with balanced class weighting."""
    ...
    def fit(self, texts: list[str], labels: list[int]) -> None:
        X = self._vectorizer.fit_transform(texts)
        # MLPClassifier lacks class_weight; approximate it by oversampling each
        # class proportionally to its balanced weight (MLP.fit takes no sample_weight).
        sample_weights = compute_sample_weight("balanced", labels)
        import numpy as np
        rng = np.random.RandomState(42)
        # Dividing by the min weight pins the largest (smallest-weight) class at 1x
        # and rounds the rarer classes up to integer duplication factors, so the
        # oversampled copies approximate balanced class weighting.
        ...
        # early_stopping must stay off here: sklearn would carve its validation
        # split out of the *oversampled* matrix, so duplicated minority rows would
        # straddle train/val and produce an optimistic, leaky stopping signal.
...
class TransformerFrozenHead:
    ...
    def _load_encoder(self):
        ...
        try:
            ...
            raise RuntimeError("Transformer benchmarks require PyTorch. Install the benchmark extras first.") from error
