...
This module calls the Gemini LLM via its REST API for conservative trade
intent confirmation. It exposes two APIs:

1. Extractive interpretation -- retained for offline/manual analysis tools.
...
class GeminiFallbackInterpreter:
    ...
    async def _generate_json(self, *, system_text: str, user_text: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Collapse any transport/HTTP/JSON-parse failure to None so callers
            # (interpret/confirm_intent) can tell an API/system failure apart
            # from a genuine Gemini answer.
            return None
...
def _apply_safety_overrides(
    *,
    ...
    # ADD with no open position can't scale into anything, so promote it to a
    # fresh entry (or drop it) here; ADD on an existing position falls through
    # to the management branch below and inherits the open position's side.
