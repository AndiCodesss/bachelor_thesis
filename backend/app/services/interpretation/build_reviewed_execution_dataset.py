"""Merge multiple reviewed_examples.jsonl inputs into the final classifier-ready execution training set.

Later inputs override earlier ones on the same transcript line, so corpora are layered in priority order.
"""
...
def _side_for_add(example: dict) -> str | None:
    # Precedence: a human-corrected original/reviewed side wins over the inferred
    # position_side, which in turn beats the looser last_side fallback — so the most
    # authoritative signal about the position's direction decides the ADD's entry side.
...
def _normalize_example(*, example: dict, dataset_path: Path) -> tuple[dict | None, str | None]:
    """Remap a reviewed example's label into the classifier's execution label space.

    SETUP_* labels collapse to NO_ACTION because spotting a setup is not yet an execution
    the copilot should act on. ADD is rewritten to a directional ENTER_LONG/ENTER_SHORT from
    the inferred position side, since the classifier only knows directional entries (no generic
    "add"). Labels already in the output space pass through; anything else is dropped.
    """
    try:
