...
4. LOCAL CLASSIFIER OVERLAY -- A frozen ModernBERT encoder with a trained head can veto
...
5. GEMINI CONFIRMATION GATE -- Optional entry intents are sent to the
   Gemini LLM for a second opinion before execution. A fail-open mechanism
   allows strong rule signals through when Gemini is unavailable.
...
State is maintained per session in FlowState objects that track recent
fragments, the candidate window, and duplicate suppression.
"""
...
from app.models.domain import ActionTag, EventType, StreamSession, TradeIntent, TradeSide, TranscriptSegment
...
from app.services.interpretation.price_resolver import _resolve_price
from app.services.interpretation.transcript_normalizer import apply_trading_asr_corrections

# Re-exported for callers/tests that import the price resolver from this module.
__all__ = ["RuleBasedTradeInterpreter", "_resolve_price"]
...
# Viewer-advice phrasing is treated as commentary in the single execution path.
ADVISORY_NON_ACTIONABLE_TRADE_PATTERNS = [
...
# The "at" entry pattern must not capture the price in a "stop at N" clause as
# the entry price (negative lookbehinds reject "stop at"/"stops at").
ENTRY_PATTERNS = [
    r"(?<!stop )(?<!stops )\bat (?P<price>[\w\.\s]+)",
    r"\bfrom (?P<price>[\w\.\s]+)",
    r"\bthrough (?P<price>[\w\.\s]+)",
]
...
class RuleBasedTradeInterpreter:
    ...
    adjacent segments and suppress duplicate intents.
    """
    ...
    def interpret_partial(self, session: StreamSession, segment: TranscriptSegment) -> TradeIntent | None:
        ...
        if intent.tag not in {
            ...
            return None
        # A management action (trim/exit/stop) is meaningless with no open
        # position; do not surface it as a partial candidate for the UI.
        management_tags = {
            ActionTag.trim,
            ActionTag.exit_all,
            ActionTag.move_stop,
        ...
        if intent.tag in management_tags and session.position is None:
            return None
    ...
    def _parse_compiled(
        ...
        *,
        ...
        setup. Returns the first matching intent, or None.
        """
        text = context.normalized
        if session.position is not None:
        ...
        if _matches_any(TRIM_PATTERNS, entry_text) or _matches_any(RUNNER_PATTERNS, entry_text):
            if mutate_state:
                state.last_management_at = context.received_at
            quantity_hint = "most" if "most my size" in entry_text or _matches_any(RUNNER_PATTERNS, entry_text) else self._detect_trim_quantity(entry_text)
            return self._build_intent(
                session=session,
                context=context,
                tag=ActionTag.trim,
        ...
        # --- Stop adjustment on an already-open position. Placed after entry
        # detection so a stop price stated alongside a fresh entry is read as
        # that entry's stop rather than as a standalone stop move. ---
    ...
    def _is_contextual_in_trade_phrase(self, text: str, *, side: TradeSide) -> bool:
        """Detect an actionable "I'm in this long/short" entry vs commentary."""
        side_word = "long" if side == TradeSide.long else "short"
        ...
        if re.search(ENTRY_BARE_CONTEXT_PATTERN, text):
            return True
        # Once the anchored/inline "in this <side>" phrasing has cleared the
        # negative-pattern filter, treat it as a real entry by default -- the
        # disqualifying commentary forms were already rejected above.
        return True
    ...
    def _detect_trim_quantity(self, text: str) -> str | None:
        """Quantity for a TRIM (partial exit). A trim must never resolve to
        'all'/'whole position' -- that would liquidate the entire position on a
        take-some-profit cue. Only an explicit EXIT_ALL performs a full close.
        """
        quantity = self._detect_quantity(text)
        return "most" if quantity == "all" else quantity
    ...
    def _restore_state_after_veto(self, session_id: str, state_before: FlowState) -> None:
        ...
        # Also roll back the stitching / candidate-window / instrument state so a
        # vetoed segment does not bias how the next segment is stitched or scored.
        state.recent_text = state_before.recent_text
        state.recent_text_at = state_before.recent_text_at
        state.active_instrument = state_before.active_instrument
        state.active_instrument_at = state_before.active_instrument_at
        state.recent_fragments = list(state_before.recent_fragments)
        state.candidate_window = state_before.candidate_window
    ...
    def _is_non_actionable_trade_commentary(self, text: str, *, session: StreamSession) -> bool:
        if _matches_any(NON_ACTIONABLE_TRADE_PATTERNS, text):
            return True
        if _matches_any(ADVISORY_NON_ACTIONABLE_TRADE_PATTERNS, text):
            return True
        return False
...
def _compact_repeated_trade_text(text: str) -> str:
    ...
    for chunk in chunks:
        ...
        if normalized_chunk == last_normalized:
            ...
            repeat_count = 1
        # Short chunks (<=4 words, e.g. "long here") repeating are almost always
        # ASR stutter, so collapse after a single repeat; longer sentences may
        # legitimately recur, so allow them to appear twice before dropping.
