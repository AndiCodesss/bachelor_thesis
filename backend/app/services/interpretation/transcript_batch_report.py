"""Replays transcripts through RuleBasedTradeInterpreter to audit which lines
fire intents and to surface likely false positives (the "suspicious" hits)."""
...
# Hypothetical, second-person, or past-tense phrasings that probably should NOT
# have produced an intent — matching one flags the hit as a likely false positive.
...
async def _replay_file(
    ...
    *,
    ...
    """Replay one transcript, simulating position state across its lines so that
    management intents (trim/exit/move-stop) interpret with realistic context."""
