...
def _rebuild_prompt(example: dict) -> str:
    ...
    market_price is intentionally dropped: baking a fixed anchor price into the
    prompt would leak that exact number into training data and teach the model to
    lean on a stale absolute price instead of the relative recent/analysis context.
    """
    parts = [
