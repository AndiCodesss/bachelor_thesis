...
{
    // HTTP listener hosted inside NinjaTrader that lets the Python backend drive a
    // live account. It receives trade commands (ENTER/ADD/TRIM/EXIT/MOVE_STOP) plus a
    // state/health query, marshals every order submission onto the NinjaTrader
    // dispatcher thread (account APIs are not thread-safe), and manages bracket
    // (stop/target OCO) protection orders that follow each position.
    // Routes: POST /api/stream-copier/commands, GET /api/stream-copier/state,
    ...
    public class StreamCopierBridgeAddOn : AddOnBase
    {
        ...
        // For WSL backend connectivity, you may bind to all local interfaces
        // (http://*:18080/), but then RequiredBearerToken must be set.
        private const string ListenerPrefix = "http://127.0.0.1:18080/";
        ...
        // How long a processed intent_id is remembered for de-duplication.
        private const int ProcessedIntentTtlMs = 60000;

        private readonly object sync = new object();
        // Pending protection intents per (account|instrument), held as a FIFO list so
        // multiple in-flight entries (e.g. rapid ENTER + ADD where both register
        // protection before either fills) do not overwrite each other. We match by
        // SignalName when the order reference is available (OnExecutionUpdate,
        // OnOrderUpdate) and fall back to FIFO for position-level events.
        private readonly Dictionary<string, List<PendingProtection>> pendingProtections = new Dictionary<string, List<PendingProtection>>(StringComparer.OrdinalIgnoreCase);
        ...
        // The Account objects we attached handlers to, so StopBridge can detach
        // them and let NinjaTrader release this AddOn instance on shutdown.
        private readonly List<Account> subscribedAccountObjects = new List<Account>();
        // intent_id -> first-seen time, so a non-idempotent command (a duplicate
        // POST, a retry, or a timed-out dispatch that still ran) executes at most once.
        private readonly Dictionary<string, DateTime> processedIntents = new Dictionary<string, DateTime>(StringComparer.OrdinalIgnoreCase);
        ...
        private void StopBridge()
        {
            ...
            // Detach account event handlers so NinjaTrader's long-lived Account
            // objects do not keep this AddOn instance (and stale handlers) alive
            // after the bridge is stopped.
            lock (sync)
            {
                foreach (var account in subscribedAccountObjects)
                {
                    account.OrderUpdate -= OnOrderUpdate;
                    account.ExecutionUpdate -= OnExecutionUpdate;
                    account.PositionUpdate -= OnPositionUpdate;
                }
                subscribedAccountObjects.Clear();
        ...
        private BridgeResponse DispatchCommand(BridgeCommand command)
        {
            ...
            Action run = () =>
            {
                try
                {
                    // If the caller already gave up (dispatch timed out), do not
                    // submit the order -- avoids a phantom fill the backend thinks failed.
                    if (Interlocked.CompareExchange(ref cancelled, 0, 0) != 0)
                    {
                        result = BridgeResponse.Error("dispatch_cancelled", "Command abandoned after dispatch timeout.");
                        return;
                    }
        ...
        private BridgeResponse ExecuteCommand(BridgeCommand command)
        {
            ...
            // Idempotency guard: a trade command must execute at most once per
            // intent_id. A duplicate (retry, or a dispatch that timed out on the
            // backend but still ran here) is acknowledged without re-submitting.
            if (!string.IsNullOrWhiteSpace(command.IntentId))
            {
                lock (sync)
                {
                    PruneProcessedIntents();
                    if (processedIntents.ContainsKey(command.IntentId))
                        return BridgeResponse.OkResult("duplicate_ignored", "Duplicate intent ignored: " + command.IntentId);
                    processedIntents[command.IntentId] = DateTime.UtcNow;
                }
            }
        ...
        private BridgeResponse SubmitEntry(Account account, Instrument instrument, OrderAction action, BridgeCommand command)
        {
            ...
            try
            {
                if (hasProtection)
                {
                    lock (sync)
                    {
                        ...
                        List<PendingProtection> queued;
                        if (!pendingProtections.TryGetValue(protectionKey, out queued))
                        {
                            queued = new List<PendingProtection>();
                            pendingProtections[protectionKey] = queued;
                        }
                        queued.Add(new PendingProtection
                        {
            ...
            catch (Exception ex)
            {
                if (hasProtection && !string.IsNullOrEmpty(protectionKey))
                {
                    lock (sync)
                    {
                        RemovePendingProtectionBySignal(protectionKey, signalName);
                    }
                }
        ...
        private BridgeResponse SubmitAdd(Account account, Instrument instrument, BridgeCommand command)
        {
            ...
            {
                string normalizedSide = command.Side.Trim().ToUpperInvariant();
                if (normalizedSide != "LONG" && normalizedSide != "SHORT")
                    return BridgeResponse.Error("bad_request", "ADD side must be LONG or SHORT when supplied.");
        ...
        private BridgeResponse SubmitExitAll(Account account, Instrument instrument)
        {
            try
            {
                // Drop any queued protection intents first so a fill/position event
                // during the flatten cannot re-bracket a position we are closing.
                lock (sync)
                    pendingProtections.Remove(BuildProtectionKey(account, instrument));
        ...
        private BridgeResponse SubmitMoveStop(Account account, Instrument instrument, double? stopPrice)
        {
            ...
            try
            {
                ...
                NormalizeStopPrice(instrument, position, ref normalizedStop);
                if (!normalizedStop.HasValue || normalizedStop.Value <= 0)
                    return BridgeResponse.Error("bad_request", "Unable to normalize stop_price for MOVE_STOP.");

                Order targetOrder;
                var stopOrder = FindProtectionOrders(account, instrument, position, out targetOrder);
                if (stopOrder == null || IsTerminal(stopOrder))
                    return BridgeResponse.Error("stop_order_not_found", "No active stop order found to move.");

                stopOrder.StopPriceChanged = normalizedStop.Value;
                ...
                lock (sync)
                {
                    ...
                    if (!protectionBySymbol.TryGetValue(key, out state) || state == null)
                        state = new ProtectionState();
                    state.StopOrder = stopOrder;
                    if (targetOrder != null)
                        state.TargetOrder = targetOrder;
                    state.StopPrice = normalizedStop;
                    if (state.TargetOrder != null)
                        state.TargetPrice = state.TargetOrder.LimitPrice;
                    protectionBySymbol[key] = state;
                }

                return BridgeResponse.OkResult("stop_updated", "Stop moved to " + normalizedStop.Value.ToString("0.00") + ".");
            }
            catch (Exception ex)
            {
        ...
        private void SubscribeAccountEvents(Account account)
        {
            lock (sync)
            {
                ...
                subscribedAccountObjects.Add(account);
            }
        }
        ...
        private void OnOrderUpdate(object sender, OrderEventArgs e)
        {
            ...
            // If an entry order is rejected/cancelled, drop only this order's pending
            // protection so it cannot leak into a later position; sibling pending
            // entries for the same instrument must remain intact.
                ...
                && IsEntryOrderAction(e.Order.OrderAction))
            {
                ...
                string signalName = e.Order.Name;
                if (!string.IsNullOrEmpty(signalName))
                {
                    lock (sync)
                    {
                        RemovePendingProtectionBySignal(key, signalName);
                    }
                }
            }
        ...
        private void OnExecutionUpdate(object sender, ExecutionEventArgs e)
        {
            ...
            string signalName = e.Execution.Order != null ? e.Execution.Order.Name : null;
            PendingProtection pending = null;
            lock (sync)
            {
                pending = TakePendingProtection(key, signalName);
            }
            ...
            try
            {
                ...
                if (position == null || position.MarketPosition == MarketPosition.Flat || position.Quantity == 0)
                {
                    ...
                    lock (sync)
                    {
                        ReinsertPendingProtection(key, pending);
                    }
                    return;
                }
        ...
        private void OnPositionUpdate(object sender, PositionEventArgs e)
        {
            ...
            lock (sync)
            {
                // Consume the oldest pending protection only once, when position becomes
                // non-flat. PositionEventArgs has no order reference, so we cannot match
                // by SignalName here; the FIFO order matches submission order.
                if (e.MarketPosition != MarketPosition.Flat && e.Quantity != 0)
                {
                    pending = TakePendingProtection(key, null);
                }
        ...
        private Order FindProtectionOrders(
            Account account,
            Instrument instrument,
            Position position,
            out Order targetOrder)
        {
            targetOrder = null;
            if (account == null || instrument == null || position == null)
                return null;
            ...
            Order preferredStop = null;
            Order fallbackStop = null;
            Order preferredTarget = null;
            Order fallbackTarget = null;
            lock (account.Orders)
            {
                foreach (Order order in account.Orders)
                {
                    ...
                    if (!string.Equals(order.Instrument.FullName, instrument.FullName, StringComparison.OrdinalIgnoreCase))
                        continue;
                    if (order.OrderAction != protectiveAction || IsTerminal(order))
                        continue;
                    ...
                    if (isStopType)
                    {
                        if (isBridgeStop && preferredStop == null)
                            preferredStop = order;
                        else if (fallbackStop == null)
                            fallbackStop = order;
                    }
                    else if (isTargetType)
                    {
                        if (isBridgeTarget && preferredTarget == null)
                            preferredTarget = order;
                        else if (fallbackTarget == null)
                            fallbackTarget = order;
                    }
                }
            }

            targetOrder = preferredTarget ?? fallbackTarget;
            return preferredStop ?? fallbackStop;
        }

        // Clamp the bracket onto the valid side of the current market: stop below
        // market for a long / above for a short, target on the opposite side, each
        // pushed out by at least minGap. NinjaTrader rejects protective orders placed
        // on the wrong side of, or too close to, the market, so a price that violates
        // this is rebuilt from the average entry instead of being passed through.
        ...
        // Stop-only variant of the same clamp: keep the stop below market for a long
        // / above market for a short, pushed out by at least minGap, because
        // NinjaTrader rejects a stop on the wrong side of, or too close to, the market.
        private static void NormalizeStopPrice(Instrument instrument, Position position, ref double? stopPrice)
        {
            if (instrument == null || position == null || position.MarketPosition == MarketPosition.Flat || !stopPrice.HasValue)
                return;
            ...
            double tickSize = ResolveTickSize(instrument);
            double avg = position.AveragePrice;
            double defaultDistance = tickSize * 40.0;
            bool isLong = position.MarketPosition == MarketPosition.Long;
            if (market.HasValue)
            {
                double minGap = tickSize * 2.0;
            ...
            stopPrice = RoundToTick(instrument, stopPrice.Value, !isLong);
        }
        ...
        private static OrderAction ParseSideToEntryAction(string side)
        {
            ...
            throw new ArgumentException("Unsupported side: " + side);
        }
        ...
        // Caller must hold sync. Drops intent_ids older than the de-dup TTL.
        private void PruneProcessedIntents()
        {
            if (processedIntents.Count == 0)
                return;
            DateTime cutoff = DateTime.UtcNow.AddMilliseconds(-ProcessedIntentTtlMs);
            List<string> stale = null;
            foreach (var entry in processedIntents)
            {
            ...
            if (stale != null)
            {
                foreach (string id in stale)
                    processedIntents.Remove(id);
            }
        }

        // Caller must hold sync. Removes and returns one queued PendingProtection,
        // preferring an entry whose SignalName matches; otherwise returns the oldest.
        private PendingProtection TakePendingProtection(string protectionKey, string signalName)
        {
            List<PendingProtection> queued;
            if (!pendingProtections.TryGetValue(protectionKey, out queued) || queued.Count == 0)
                return null;
            ...
            if (!string.IsNullOrEmpty(signalName))
            {
                ...
                {
                    if (string.Equals(queued[i].SignalName, signalName, StringComparison.OrdinalIgnoreCase))
                    {
                        idx = i;
                        break;
                    }
            ...
            PendingProtection pending = queued[idx];
            queued.RemoveAt(idx);
            if (queued.Count == 0)
        ...
        // Caller must hold sync. Re-inserts a previously taken PendingProtection at
        // the front of the FIFO so the next event re-attempts it before any newer ones.
        private void ReinsertPendingProtection(string protectionKey, PendingProtection pending)
        {
            if (pending == null)
                return;
            List<PendingProtection> queued;
        ...
        // Caller must hold sync. Removes the queued PendingProtection with the given
        // SignalName, leaving siblings for the same instrument intact.
        private void RemovePendingProtectionBySignal(string protectionKey, string signalName)
        {
            if (string.IsNullOrEmpty(signalName))
                return;
            List<PendingProtection> queued;
        ...
        private bool IsAuthorized(HttpListenerRequest request)
        {
            ...
            return FixedTimeEquals(auth, expected);
        }

        // Constant-time string comparison so a bearer-token check cannot be
        // brute-forced byte-by-byte via response timing. (.NET Framework 4.8 has
        // no CryptographicOperations.FixedTimeEquals, so compare manually.)
