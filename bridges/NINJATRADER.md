...
It is designed to pair with this backend's NinjaTrader executor.

## Install

1. Copy [StreamCopierBridgeAddOn.cs](/mnt/c/Users/Andreas Oberdörfer/Downloads/stream_copier/bridges/StreamCopierBridgeAddOn.cs) to:
...
- `RequiredBearerToken` (must match backend `NINJATRADER_BRIDGE_TOKEN`; required for non-loopback bindings)
...
1. Use wildcard listener in AddOn (`ListenerPrefix = "http://*:18080/"`) and set `RequiredBearerToken`.
