...
**Final model** (`distil-small.en` by default, beam size 2): Runs once the segmenter says a speech segment is complete, using higher-accuracy decode settings than the preview (larger beam, a repetition penalty, and an anti-hallucination retry). It replaces the preview with the "real" transcription. The model is selectable per session from the dashboard (`distil-small.en` or `small.en`); the default stays small so transcription keeps up with live speech instead of falling behind. Heavier models like `distil-large-v3` are intentionally not offered in the UI because on CPU they run slower than real time and make the pipeline lag.
...
The foundation. Contains 192 regex patterns organized by action type:

**Entry patterns** (23 long + 25 short + 31 side-neutral = 79 total): Match phrases like "I'm long", "going short", "buying here", "got my entry". Side-neutral patterns ("small piece on here", "I'm in this") use position context to determine the side. Each returns a `PhraseSignal` with the action tag and side.

**Exit patterns** (30): "I'm out", "flatten", "close it out", "stopped out", "knocked out", "done with this".
...
**Setup patterns** (6 long + 10 short = 16): "looking for a long", "watching for short setup", "interested in shorts". These are lower confidence — the trader is thinking about a trade but hasn't committed.
...
The `RuleBasedTradeInterpreter` class (the heart of a roughly 2,500-line module) ties all four layers together. It also manages:
...
A Pydantic Settings class with 74 configurable settings, read from `backend/.env`. Groups:
...
| TF-IDF + MLP | Same features → neural net with two hidden layers (256 → 128 neurons) |
