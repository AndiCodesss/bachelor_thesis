...
| Correct or acceptable | 202 (99.5%) |
| Clearly wrong | 0 (0.0%) |
| Accepted but ambiguous | 2 (subset of accepted labels) |
| **Operational label acceptance** | **99.5%** |

The two ambiguous labels were included in the 202 accepted labels. They were borderline cases where the speaker used second-person phrasing ("you got to pay yourself some here") while actively managing their own position; the intended meaning was self-directed but the wording was directed at viewers.

#### Coverage (Recall) Assessment

While accepted action labels are highly reliable, the dataset does not capture every trade action in each transcript. An automated scan for trade-related language on unlabeled lines identified approximately 230 potential unlabeled actions across the 10 transcripts. These were primarily:
...
- **Key parameters**: Hidden layers `(256, 128)`, `max_iter=300`, `early_stopping=False`
- **Characteristics**: Non-linear decision boundaries. Uses oversampling with sklearn's `compute_sample_weight("balanced")` to handle class imbalance — each sample is repeated proportionally to its balanced weight, on top of the rebalanced training set. Early stopping is disabled: it would carve its internal validation split out of the *oversampled* matrix, placing duplicated minority rows in both the training and validation halves (a leak). The measured effect of this correction was within noise (Macro F1 unchanged at 0.634; other metrics shifted by less than 0.01).
...
After applying the data cleanup pipeline, the benchmark was re-run on the cleaned dataset (1,377 examples after content-level deduplication at load time, 103 transcripts) for all five models.
...
| MLP (balanced) | 0.7625 +/- 0.0242 | 0.6339 +/- 0.0485 | 0.8502 +/- 0.0057 | 0.8981 +/- 0.0366 | 0.8089 +/- 0.0306 |
| ModernBERT | 0.7267 +/- 0.0123 | 0.6488 +/- 0.0604 | 0.8723 +/- 0.0308 | 0.8328 +/- 0.0435 | **0.9171 +/- 0.0368** |
| DistilBERT | 0.6755 +/- 0.0650 | 0.5875 +/- 0.0289 | 0.8479 +/- 0.0338 | 0.8010 +/- 0.0697 | 0.9089 +/- 0.0707 |
...
The TF-IDF-based classical models (LogReg, SVM) outperform the transformer-based models (DistilBERT, ModernBERT) on the aggregate cleaned-dataset metrics. This is likely attributable to:
...
After adding balanced class weighting via oversampling (scikit-learn's `MLPClassifier` lacks native `class_weight`), the cleaned-dataset MLP reaches 0.6339 Macro F1 and 0.8502 Action F1. Compared with the original-dataset run, cleanup plus balancing improved Macro F1 by +4.23 pp. The fix resolved the previous zero-prediction failure on `MOVE_TO_BREAKEVEN` and improved recall on entry labels. However, it still trails LogReg (0.7025 Macro F1) and SVM (0.6881), suggesting the MLP's additional capacity (256+128 hidden units) overfits on this small dataset despite balancing and a capped iteration budget. (These cleaned-MLP figures were re-run after disabling early stopping, which would otherwise leak across the oversampled validation split; the change versus the original early-stopping run was within noise.)

### 9.4 MOVE_TO_BREAKEVEN Is the Weakest Class

With only 13-15 examples, `MOVE_TO_BREAKEVEN` remains the least reliable class. After the MLP balancing fix, F1 ranges roughly from 0.13 to 0.55 depending on model, with ModernBERT highest on this rare class but LogReg still strongest overall. The earlier unbalanced MLP assigned zero predictions to this class; balancing corrected that failure, but performance remains weak and high-variance. This class requires more training examples to reach reliable performance.
...
ModernBERT benefited strongly from cleanup (+4.75% accuracy and +4.65% Macro F1), while DistilBERT showed mixed movement (+3.80% accuracy and +1.08% Action F1, but -0.66% Macro F1). This suggests that noisy position state features can be harmful to dense embeddings, which encode all input features into a single vector; a misleading feature like `position=FLAT` on a TRIM example pollutes the entire embedding, whereas TF-IDF models can partially ignore irrelevant features through their sparse representation.
...
- **EXIT_ALL** is dominated by the single unigram `"out"` (weight +5.61) — traders almost always say "I'm out" or "out of this trade."
...
|-----------|-----------|---------|------------------------|
| LogReg vs. SVM | +0.0145 | 0.541 | No |
| LogReg vs. MLP | +0.0686 | 0.158 | No |
| SVM vs. MLP | +0.0541 | 0.236 | No |
| LogReg vs. ModernBERT | +0.0538 | 0.187 | No |
| SVM vs. ModernBERT | +0.0393 | 0.436 | No |
| LogReg vs. DistilBERT | +0.1151 | **0.014** | **Yes** |
| SVM vs. DistilBERT | +0.1006 | 0.063 | No |
| MLP vs. DistilBERT | +0.0465 | 0.147 | No |
| ModernBERT vs. DistilBERT | +0.0613 | 0.111 | No |
| MLP vs. ModernBERT | -0.0149 | 0.696 | No |

### 12.2 Interpretation

- **LogReg vs. SVM**: The difference is **not significant** (p=0.541). These models perform similarly on this dataset; both are linear classifiers on identical TF-IDF features.
- **LogReg/SVM vs. MLP**: The linear models remain ahead in aggregate, but the gaps are **not significant** after the balanced MLP correction (LogReg vs. MLP p=0.158; SVM vs. MLP p=0.236).
- **Classicals vs. Transformers**: Only LogReg vs. DistilBERT is statistically significant at p<0.05 (p=0.014). The other transformer comparisons are not significant with only 5 folds, so the aggregate ranking should be interpreted together with the mechanistic evidence from feature importance and embedding visualization.
...
With only 13-15 examples depending on the dataset version, `MOVE_TO_BREAKEVEN` cannot be reliably evaluated. In 5-fold CV, each test fold contains roughly three examples of this class. F1 scores range from 0.00 in the earlier unbalanced MLP run to about 0.55 for ModernBERT on the cleaned dataset, but these numbers carry high variance. This class is retained as a separate label because it maps to a distinct broker action, but its metrics should be interpreted with caution.
...
| MLP | (256, 128) hidden, no early stopping, balanced oversampling | Two layers sufficient for non-linear boundaries on TF-IDF; early stopping is disabled because oversampled duplicates would leak into its internal validation split (a capped iteration budget of 300 bounds training instead); balanced via `compute_sample_weight` + oversampling (MLPClassifier lacks native `class_weight`) |
...
- **ModernBERT** produces an even more uniform cloud with less sub-structure — yet its classification accuracy is *higher* (0.727 vs DistilBERT's 0.676, though both trail TF-IDF). This confirms that the sub-structure visible in the DistilBERT plot is not label-aligned and carries no usable classification signal.
...
Full error dump (generated into `backend/data/misclassified.json` by the error-analysis pipeline; not committed to the repository).
...
**Finding**: Performance saturates at 10,000 features — increasing to 20,000 or 50,000 produces identical results. The actual vocabulary (with bigrams) in this dataset is approximately 10,000 unique features, so higher caps simply have no additional features to include. Even 5,000 features captures 99.7% of the top macro F1, confirming that the effective feature space is small and dominated by a relatively compact set of trading-specific terms.

---

## 17. Reproducibility

All results can be reproduced from the labelled dataset (`data/training_data.jsonl`). That file is the local input to the pipeline and is not distributed with the repository; with it in place under `backend/data/`, run:
