/*
 * audio-processor.js -- runs inside an AudioWorklet (a separate audio thread).
 *
 * The browser hands us audio in small ~128-sample blocks of 32-bit floats. The
 * backend wants 16-bit PCM in larger chunks, so this processor:
 *   1. converts each float sample (range -1..1) to a 16-bit integer, and
 *   2. buffers samples until it has `targetFrameCount` of them, then posts that
 *      chunk back to the main thread (which forwards it over the WebSocket).
 * Batching into larger chunks keeps the message rate -- and overhead -- low.
 */
class PcmCaptureProcessor extends AudioWorkletProcessor {
  ...
  process(inputs) {
    ...
    for (let index = 0; index < channel.length; index += 1) {
      // Clamp to [-1, 1] then scale to the signed 16-bit range. The factors
      // differ (32768 vs 32767) because that range is asymmetric: -32768..32767.
