/*
 * api.ts -- a thin client for the backend REST + WebSocket API.
 *
 * Each function wraps `fetch`, returns parsed JSON, and throws an Error carrying
 * the backend's message on failure. The base URLs default to localhost but can
 * be overridden with Vite env vars (e.g. VITE_API_BASE_URL) to point at another
 * host or port without touching the code.
 */
...
// Turn a fetch Response into typed data. On an error status we try to surface
// the most useful message (FastAPI returns it under `detail`); on 204 No
// Content there is no body, so we return undefined.
...
// Create a session with sensible defaults for this project (the trader we
// follow, partial-intent preview on, cloud AI off). The backend fills in the
// rest of the config.
