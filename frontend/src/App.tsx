/*
 * App.tsx -- the whole Stream Copier dashboard lives in this one component.
 *
 * Responsibilities:
 *  - List / create / delete trading sessions and pick the active one.
 *  - Capture audio from a shared browser tab, downsample it in an AudioWorklet,
 *    and stream it to the backend over a WebSocket (with auto-reconnect).
 *  - Subscribe to a second WebSocket that pushes live session updates (new
 *    transcripts, detected intents, risk decisions, executions) and apply those
 *    deltas to local state.
 *  - Poll the broker for live position / PnL telemetry.
 *  - Offer manual controls: inject transcript text, place manual trades, and
 *    toggle interpretation features.
 *
 * The component is intentionally "flat" (no global state library): React state
 * holds everything, and the two WebSockets plus one polling loop keep it fresh.
 */
...
// How much audio to buffer before sending a chunk. ~30 ms keeps latency low
// while still batching enough samples to be efficient over the WebSocket.
const AUDIO_CHUNK_TARGET_MS = 30;
// Reconnect backoff for the audio socket: start at 750 ms and grow linearly,
// capped at 3 s, so a flaky connection retries without hammering the backend.
const AUDIO_RECONNECT_BASE_MS = 750;
const AUDIO_RECONNECT_MAX_MS = 3_000;
// Broker polling cadence adapts to activity: fast while capturing, medium when
// a position is open, slow when idle -- balancing freshness against load.
...
function App() {
  // --- React state: the full UI is derived from these values ---
  ...
  // --- Refs hold things that must survive re-renders without triggering them:
  //     the two WebSockets, the Web Audio graph, and the reconnect bookkeeping. ---
  ...
  // On first mount, load any sessions that already exist on the backend and
  // select the first one so the dashboard isn't empty on a page refresh.
  useEffect(() => {
  ...
  // Keep the active selection valid: clear it when no sessions remain, and
  // fall back to the first session if the selected one was deleted elsewhere.
  useEffect(() => {
  ...
  // Open the "events" WebSocket for the active session. The backend sends one
  // full snapshot on connect, then small patch messages for each change. We
  // apply those patches locally so the UI updates in real time without polling.
  useEffect(() => {
    if (!activeSessionId) {
      return undefined;
    }
  ...
  // Mirror the active session's broker overrides into the editable inputs
  // whenever the session changes, so the fields show the current routing.
  useEffect(() => {
    if (!activeSession) {
      setBrokerAccount("");
      setBrokerSymbol("");
  ...
  // Debounced save of the broker account/symbol overrides: wait 300 ms after
  // the user stops typing, and skip the network call if nothing actually
  // changed, so we don't PATCH the backend on every keystroke.
  useEffect(() => {
  ...
  // Poll the broker for live position/PnL telemetry. This is a self-scheduling
  // loop (setTimeout chained in `finally`) rather than setInterval so a slow
  // response can't pile up overlapping requests. The `closed` flag prevents a
  // late response from updating state after the effect has been cleaned up.
  useEffect(() => {
    if (!activeSessionId) {
      setBrokerState(null);
  ...
  // Schedule a reconnect of the audio socket after a drop -- but only if the
  // user hasn't stopped capture and the microphone/worklet are still alive.
  // Uses linear backoff so repeated failures don't spam the backend.
  ...
  // Fully tear down a capture session: stop reconnect attempts, close the
  // socket, disconnect the Web Audio graph, stop the media tracks, and close
  // the AudioContext. Written to be safe to call from anywhere (stop button,
  // an error, or the tab-share ending) without leaking audio resources.
  ...
  // Open (or re-open) the audio WebSocket and wire up its handlers. On open we
  // send the sample rate so the backend knows how to resample; on close we try
  // to reconnect unless the user deliberately stopped.
  ...
  // Start capturing tab audio. The flow is: ask the browser to share a tab
  // (getDisplayMedia) -> keep only the audio track -> build a Web Audio graph
  // that runs our AudioWorklet to turn float samples into 16-bit PCM -> forward
  // each PCM chunk over the WebSocket. A muted gain node is connected to the
  // destination only because some browsers won't run the graph otherwise.
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="panel-header">
          <h1>Stream Copier</h1>
          <button className="primary" disabled={isCreatingSession} onClick={() => void handleCreateSession()}>
      ...
      <main className="content">
        <section className="hero">
          ...
          <div className="capture-controls">
            {/* "New Session" lives in the sidebar; here we focus on capture. */}
            <button
              className="primary"
...
// `prepend` is used when creating a session so the newest appears at the top.
...
// Apply one delta message from the events WebSocket to a session. The backend
// only sends the fields that changed (SessionPatch with exclude_unset), so we
// check `in patch` for each field: present means "update it", absent means
// "leave as-is". Lists (events, transcripts) are capped so memory stays bounded
