...
A new form of real-time financial commentary has emerged from the rise of live-streaming. Day traders on YouTube and Twitch put their screens on display and narrate trades as they unfold, announcing in ordinary spoken language each purchase, each sale, each adjustment to a position, each shift of a stop-loss. Anyone hoping to follow or mirror these trades faces a plainly stated difficulty: the valuable signals sit buried inside hours of talk that also carries market analysis, chat replies, personal anecdotes, and stretches of silence. Catching the precise instant a trader commits demands unbroken attention. Look away for a moment, and a signal can pass unnoticed. And because timing so often separates a winning trade from a losing one in day trading, a signal caught too late holds little value.

Such streams are popular. When markets are open, the better-known day-trading channels pull in thousands of simultaneous viewers, a great many of them tracking one single instrument like the E-mini Nasdaq futures (NQ). Yet a streamer's spoken words, unlike the written trade alerts that circulate on social media, are fleeting and unstructured. Telling apart mere thinking out loud (``I'm watching this level'') from a genuine commitment (``I'm long here'') can be a fine distinction, and it hinges on context.

This is the motivation for an automated system with three jobs: (1)~turn the live audio into text as it happens, (2)~decide which of those text lines genuinely carry a trade signal, and (3)~sort each signal into a concrete action, such as ``enter long,'' ``trim position,'' or ``move stop.'' Recent large models like Whisper~\cite{radford2023whisper} have transformed that first step, converting speech to text at an accuracy close to a human transcriber. Steps two and three pose a text classification problem: from a short transcript line and the current trading context, pick one label out of a fixed list.

Fundamentally this is text classification, among the most heavily studied problems in natural language processing (NLP)~\cite{kowsari2019text}. Methods of the classical sort, which count words and pass them to a simple linear model, have long performed well at jobs like separating positive reviews from negative ones, or spam from legitimate email~\cite{joachims1998text}. The recent arrival of large pretrained models called transformers, BERT among them~\cite{devlin2019bert}, has set fresh records across many NLP benchmarks; these models learn rich, context-aware representations of words from enormous volumes of text. Bigger, though, is not always better. On small, specialised datasets the advantage can evaporate, because a model trained on general text may simply overlook the niche vocabulary that actually matters here~\cite{kowsari2019text}. Hence the central question: for a task like this one, is a transformer's added complexity worth the cost?

This domain throws several problems at you at once. The language, to begin with, is casual and steeped in trader slang (``peel some off,'' ``add to the runner,'' ``move to breakeven''). The speech-to-text stage then adds errors of its own: specialist terms and sound-alike words get misheard often, altering how the text appears to a classifier; concrete examples appear in Section~\ref{sec:asr}. There is also severe class imbalance, since most of what a trader says is commentary rather than a real trade call. A fourth issue is that one action can be phrased in countless ways; a trader going long might say ``I'm long here,'' ``let's try it,'' ``small size,'' or merely ``here we go.'' And the dataset assembled for this work is small by today's NLP standards, holding fewer than 1.500 labelled examples. These traits together make the task a fitting test case for pitting classical and modern model families against each other under realistic conditions.
...
Answering it means placing five models, three classical and two transformer-based, on equal footing: identical labelled data for training and testing, an identical scoring procedure, so that whatever separates the results traces back to the model rather than the setup. Two further questions accompany the main one. First, among the trading actions, which yield most readily to automatic detection, which resist it, and for what reason? Second, when a model misreads a line, what sorts of errors recur, and how do they connect to the way traders talk?
...
The groundwork for this thesis rests on a set of methods and research areas, surveyed here. The path runs from broad ideas in text classification toward the particular model families under evaluation, turns next to the speech recognition technology behind the input data, and closes with a tour of how NLP is applied across financial markets.

\subsection{Text Classification}
\label{sec:text_classification}

Text classification, the assignment of predefined labels to natural-language documents, is a foundational NLP task; it stretches across spam filtering, sentiment analysis, topic categorization, and intent detection~\cite{kowsari2019text}. What mainly sets approaches apart is how the text gets represented (engineered features versus learned representations) and how complex the classifier is (linear versus nonlinear).

In their early opinion-mining work, Hu and Liu~\cite{hu2004mining} showed that sentiment could be pulled from unstructured reviews using fairly simple pattern-based and statistical methods. The lesson was that domain-specific lexical patterns are frequently highly predictive, even with no deep syntactic or semantic analysis behind them. Later surveys take a longer view. Percha's clinical-text-mining review~\cite{percha2021modern}, for instance, traces the path from rule-based systems through to neural methods, while insisting that the data, not the novelty of a method, should drive the choice. One finding recurs: the quality and representativeness of the training data tend to matter more than the algorithm~\cite{kowsari2019text}. Small, noisy, or imbalanced datasets can wipe out the edge of sophisticated models, whereas careful curation and feature engineering can make up for a plain architecture. This anticipates the central tension of the present work, namely whether the richer representations of pretrained transformers overcome the handicap of a small, noisy, domain-specific corpus, or whether the corpus characteristics win out.
...
\textbf{Multi-Layer Perceptrons} (MLPs) are a feedforward architecture that stacks an input layer, then one or more hidden layers whose activation functions are nonlinear, and finally an output layer~\cite{goodfellow2016deep}. For an MLP with a single hidden layer, the computation is:
...
\textbf{ModernBERT}~\cite{warner2024modernbert} is a recent redesign of BERT that includes many improvements developed since the original appeared. It can read much longer inputs at once (up to about 8.000 tokens instead of a few hundred), it uses newer and more efficient ways of paying attention to context, and it was trained on far more data, about 2 trillion tokens of text and code, an order of magnitude more than BERT. It is larger than the original BERT (149 million parameters) yet still runs faster, thanks to these efficiency changes, and it sets new top scores among models of its type. The technical labels behind these gains, such as rotary position embeddings~\cite{su2024roformer} and faster attention~\cite{dao2022flashattention}, do not matter much here. What counts for this thesis is simpler: ModernBERT is a stronger, more modern encoder than DistilBERT.
...
Whisper~\cite{radford2023whisper} is OpenAI's large speech-to-text model, trained on more than 680.000 hours of audio gathered from the internet. Two stages do the work: one part listens to the audio and converts it into an internal numerical form, while the second reads that form and produces the text word by word. Its training data is ``weakly supervised,'' which means the matching transcripts were scraped from the web (video subtitles, for instance) rather than carefully typed up by people. Such transcripts hold some errors. The sheer volume of data, however, swamps them: the model still learns to transcribe correctly, because the mistakes are rare and inconsistent while the correct patterns recur millions of times.

Whisper generalises unusually well to audio it was never specifically trained for, such as varied accents, recording quality, and subject matter. It often matches systems tuned for a single setting, a property known as \emph{zero-shot} robustness, and on several benchmarks it comes close to human transcribers~\cite{radford2023whisper}. The model ships in several sizes, ranging from a 39-million-parameter version up to a 1,5-billion-parameter one, which trade speed against accuracy.

Whisper is not perfect, though. Like any speech-to-text system, it stumbles on specialist jargon, fast or informal speech, overlapping voices, and poor audio. An example makes the kinds of mistakes that matter here easiest to see. Picture a trader speaking quickly and mid-thought:

\begin{quote}
``okay, VWAP's right here, I'm gonna peel some off and move to breakeven.''
\end{quote}

A clean transcript would preserve every word. In practice, several things can go wrong with this single line. The technical term ``VWAP'' (a common price indicator), heard only rarely by the model, might surface as ``the WAP'' or ``v-wap.'' A perfectly ordinary word can also be swapped for its sound-alike: ``peel'' (meaning to sell part of a position) might turn into ``peal.'' And because the trader is rushing, filler words and half-finished phrases get merged or dropped, so ``I'm gonna peel some off'' may shrink to something shorter and vaguer. Whisper, on top of all this, chops the audio into chunks, so one sentence can be cut in two, leaving ``I'm'' at the end of one line and ``long here'' at the start of the next, which makes that line impossible to classify on its own. To reduce this, the full system stitches neighbouring chunks back together.
...
A large and still-growing body of research applies natural language processing to financial data. One early contribution, from Loughran and McDonald~\cite{loughran2011liability}, showed that general-purpose sentiment dictionaries falter on financial text: a word such as ``liability'' reads as negative in everyday language yet carries neutral or even positive meaning in a financial setting. That finding spurred first the building of domain-specific lexicons and, later, of domain-specific language models.

Aggregate mood states extracted from Twitter (today's X) were shown by Bollen et al.~\cite{bollen2011twitter} to anticipate movements in the Dow Jones Industrial Average, tying textual sentiment to market behavior. Transformer-based models have since been pointed straight at financial text. FinBERT~\cite{yang2020finbert}, a BERT model given further pretraining on financial communications, reaches strong performance on financial sentiment classification tasks. For the broader field, the surveys of Xing et al.~\cite{xing2018natural} and Wankhade et al.~\cite{wankhade2022sentiment} map the many NLP applications across finance and sentiment analysis, spanning news-based prediction through earnings-call analysis.

In one important respect, the present work parts from this prior literature. Where earlier studies analyze written financial texts (news articles, regulatory filings, social media posts), the focus here is \emph{spoken} trading commentary transcribed in real time. The input is no polished written document. It is a noisy, fragmentary transcript of rather spontaneous speech, and that brings challenges more characteristic of dialogue systems and spoken language understanding than of traditional financial NLP.
...
The present work focuses exclusively on stage~3: given a transcript segment and its trading context, predict the correct action label. Every other stage is treated as fixed infrastructure. The complete pipeline architecture is illustrated in Figure~\ref{fig:pipeline}.
...
\textbf{Stage 1: AI labelling.} Large language models (LLMs) processed trading livestream transcripts archived from YouTube, assigning an action label to each transcript line from the speaker's words and trading context. Three LLMs were used across different annotation passes. Claude Sonnet produced 278 labelled examples in an initial high-quality pass, Gemini Pro added 78 examples in a validation run, and Gemini Flash produced 1.139 examples in a bulk labelling pass. The approach follows a research-literature idea called weak supervision: rather than having humans label every line, an automatic but imperfect labeller handles the bulk of the work first, and humans then check and correct it~\cite{ratner2017snorkel}. Such automated annotation makes mistakes, as any does: an LLM can mislabel sarcasm, hypothetical talk, or a half-finished sentence. The stages that follow catch many of these errors, though not all.
...
\begin{table}[H]
\centering
\caption{How the labels are distributed across the original dataset (1.466 file entries; 1.407 unique examples once content duplicates are removed).}
...
The largest fix concerns contradictory position states. Some management-label examples (for example \texttt{TRIM} or \texttt{EXIT\_ALL}) carried \texttt{position=FLAT}, which is impossible: a trader cannot trim a position that does not exist. These contradictions survived the human review because the review judged whether the action label matched the trader's words; the position fields were filled in automatically during labelling and were not part of that check. For 200 examples, other recorded fields exposed the true position, so the state could be repaired. The few contradictory examples that resisted repair were dropped.

Two further, smaller categories fell away in cleanup. One was duplicate announcements. Because speakers on a livestream tend to restate themselves, a single action could surface under two labels only a few lines apart (``piece off into 50s'', then ``partial here, folks'' two lines on, both refer to the same trim), and the rule kept only the earliest mention. The other category was talk aimed at the audience: a handful of lines coach the viewers instead of reporting the trader's own position (``you must pay yourself here''). Treating those as real executions would train the classifier toward precisely the false positive the system is built to suppress. \textbf{1.442 clean examples} drawn from 103 transcripts survive once everything is done.

A final cull runs on its own each time the data loads. Whenever an entry matches another character for character across its label, prompt text, and position fields, the two are merged into a single record. These duplicates appear because overlapping chunks of speech can yield the identical prompt twice; left in place, a line would be counted twice over during training and testing. The effect is to reduce the original dataset from 1.466 to 1.407 unique examples and the cleaned dataset from 1.442 to 1.377.
...
The first baseline is logistic regression, left in the default configuration of its scikit-learn implementation. Two settings matter. L2 regularization is applied at a moderate strength, which keeps any one word from controlling the outcome, and class weights are balanced, which makes the rare classes weigh more heavily during training. No model in the comparison is simpler; it emits probabilities on its own and copes with the class imbalance without extra work.
...
\textbf{Accuracy} is the fraction of examples classified correctly. Simple, yes, but it can deceive on imbalanced data: a classifier that always guesses \texttt{NO\_ACTION} would reach about 39\% accuracy here without catching a single trade.

\textbf{Macro F1} is the unweighted mean of the per-class F1 scores, giving each of the seven classes equal say. A model that neglects the rare classes is visibly penalized by this metric, which makes it a fairer assessment than plain accuracy. Precision is the share of a model's predictions for a class that prove correct; recall is the share of that class's true examples the model actually finds. F1 itself is the harmonic mean of precision and recall:
...
Despite the extreme rarity of this class, ModernBERT reaches the highest F1 on \texttt{MOVE\_TO\_BREAKEVEN} (0,546) among all five models. The likely reason is that its contextual embeddings pick up some similarity between ``breakeven'' language and other stop-management language. This edge on a single rare label does not, however, carry over into the best aggregate score.
...
Second, this caution has a price. Of the 23 labelled actions, the pipeline catches only 7, well short of the standalone model's 18, because the rule engine turns down exactly the vague phrasings (``Okay. Out. Oh, let it squeeze out.'') that the statistical model would catch. But everything it does flag, it labels correctly, every time. The system is built to miss a trade rather than invent one, and the numbers show that choice clearly.
...
This tradeoff carries direct practical consequences, because the two error types cost different things. A false positive means executing a trade the trader never made; a false negative means missing a trade he did make. A deployment that prioritizes safety would favour a precision-leaning model such as the SVM, accepting that some signals are missed. One that prioritizes completeness would favour high recall, that is, the transformers, accepting more false alarms that must then be caught by downstream safeguards. The balanced profile of logistic regression makes it the most versatile default.

\subsection{Which Actions Are Hard, and Why}
\label{sec:discussion_hierarchy}

Across all five models the per-label results tell one consistent story. Management actions (\texttt{MOVE\_STOP}, \texttt{EXIT\_ALL}, \texttt{TRIM}) are easiest, landing at F1 around 0,70--0,85, since traders announce them with explicit, almost ritual words: ``stop,'' ``out,'' ``partial.'' The \texttt{NO\_ACTION} class falls in between (F1 roughly 0,72--0,83) and varies from model to model, because the line between no action and a subtly phrased action is genuinely blurry, the more so given the labels' incomplete action coverage. Entries (\texttt{ENTER\_LONG}, \texttt{ENTER\_SHORT}) are consistently hard (F1 roughly 0,48--0,64): entry language is varied and often implicit, and the model must also pin down the direction, which can hide in an earlier sentence. \texttt{MOVE\_TO\_BREAKEVEN} remains the unstable outlier, for the reason already established: there is almost no data for it.
...
\textbf{Training with realistic class balance.} In the training data the model sees one action for every 2,5 non-actions. A real stream is far more uneven, with over a hundred non-action lines per action, and the full-stream test (Section~\ref{sec:fullstream}) showed exactly where that mismatch leads. The most direct route to closing the measured precision gap is retraining on far more hard negative examples, precisely the analysis-with-trade-words chatter that triggers the false alarms.
