...
# One-command local dev launcher: provisions a Python venv, installs the backend
# (plus the optional intent-classifier and optional CUDA runtime when a GPU is
# present), picks free ports, and runs the uvicorn backend and Vite frontend
# together so a single Ctrl+C tears both down.
# Honored env vars: BACKEND_PORT and FRONTEND_PORT override the default ports
# (free-port search starts from them), STREAM_COPIER_VENV_DIR overrides where the
# venv lives.
...
default_venv_dir() {
  ...
  # /mnt/* means the project sits on a WSL-mounted Windows drive, where venv I/O
  # (lots of small files) is painfully slow. Relocate the venv onto the native
  # Linux filesystem, keyed by a hash of the project path so each checkout gets
  # its own.
...
find_available_port() {
  ...
  while true; do
    # Skip 5173: it is Vite's default dev port, reserved so we never hand it to
    # the backend and clash with the frontend dev server.
    if [[ "$port" == "5173" ]]; then
      port=$((port + 1))
      continue
    fi
