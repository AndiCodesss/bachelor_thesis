...
MISSING_IDS_FILE = _TMP_DIR / "missing_ids.txt"
ALL_STREAMS_FILE = _TMP_DIR / "all_streams.txt"
FAILED_IDS_FILE = _TMP_DIR / "failed_transcript_ids.txt"
...
def _download_transcript(video_id: str, title: str) -> tuple[bool, str]:
    ...
    tmp_prefix = str(_TMP_DIR / f"yt_sub_{video_id}")
    try:
        result = subprocess.run(
            [
                "yt-dlp",
...
def main() -> int:
    """Read the discovery step's missing-IDs and stream-titles temp files, download each missing transcript via yt-dlp, and write any failed IDs to FAILED_IDS_FILE."""
